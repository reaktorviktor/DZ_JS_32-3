...
gmail_button.onclick = () => {
    if (regExp.test(gmail_input.value)) {
        gmail_result.innerHTML = 'Луффи принял заявку'
        gmail_result.style.color = 'green'
    }else {
        gmail_result.innerHTML = 'вы не понравились Луффи'
...
let angle14 = 11 * Math.PI

const radius = parent.clientWidth / 1.4 - 2

const childWidth = child1.clientWidth / 3
const childHeight = child1.clientHeight / 3
...
function moveCircular() {
    const x1 = parent.clientWidth / radius_x + radius * Math.cos(angle1) - childWidth
    const y1 = parent.clientHeight / radius_y + radius * Math.sin(angle1) - childHeight
    child1.style.left = x1 + 'px'
    child1.style.top = y1 + 'px'

    const x2 = parent.clientWidth / radius_x + radius * Math.cos(angle2) - childWidth
    const y2 = parent.clientHeight / radius_y + radius * Math.sin(angle2) - childHeight
    child2.style.left = x2 + 'px'
    child2.style.top = y2 + 'px'

    const x3 = parent.clientWidth / radius_x + radius * Math.cos(angle3) - childWidth
    const y3 = parent.clientHeight / radius_y + radius * Math.sin(angle3) - childHeight
    child3.style.left = x3 + 'px'
    child3.style.top = y3 + 'px'

    const x4 = parent.clientWidth / radius_x + radius * Math.cos(angle4) - childWidth
    const y4 = parent.clientHeight / radius_y + radius * Math.sin(angle4) - childHeight
    child4.style.left = x4 + 'px'
    child4.style.top = y4 + 'px'

    const x5 = parent.clientWidth / radius_x + radius * Math.cos(angle5) - childWidth
    const y5 = parent.clientHeight / radius_y + radius * Math.sin(angle5) - childHeight
    child5.style.left = x5 + 'px'
    child5.style.top = y5 + 'px'

    const x6 = parent.clientWidth / radius_x + radius * Math.cos(angle6) - childWidth
    const y6 = parent.clientHeight / radius_y + radius * Math.sin(angle6) - childHeight
    child6.style.left = x6 + 'px'
    child6.style.top = y6 + 'px'

    const x7 = parent.clientWidth / radius_x + radius * Math.cos(angle7) - childWidth
    const y7 = parent.clientHeight / radius_y + radius * Math.sin(angle7) - childHeight
    child7.style.left = x7 + 'px'
    child7.style.top = y7 + 'px'

    const x8 = parent.clientWidth / radius_x + radius * Math.cos(angle8) - childWidth
    const y8 = parent.clientHeight / radius_y + radius * Math.sin(angle8) - childHeight
    child8.style.left = x8 + 'px'
    child8.style.top = y8 + 'px'

    const x9 = parent.clientWidth / radius_x + radius * Math.cos(angle9) - childWidth
    const y9 = parent.clientHeight / radius_y + radius * Math.sin(angle9) - childHeight
    child9.style.left = x9 + 'px'
    child9.style.top = y9 + 'px'

    const x10 = parent.clientWidth / radius_x + radius * Math.cos(angle10) - childWidth
    const y10 = parent.clientHeight / radius_y + radius * Math.sin(angle10) - childHeight
    child10.style.left = x10 + 'px'
    child10.style.top = y10 + 'px'

    const x11 = parent.clientWidth / radius_x + radius * Math.cos(angle11) - childWidth
    const y11 = parent.clientHeight / radius_y + radius * Math.sin(angle11) - childHeight
    child11.style.left = x11 + 'px'
    child11.style.top = y11 + 'px'

    const x12 = parent.clientWidth / radius_x + radius * Math.cos(angle12) - childWidth
    const y12 = parent.clientHeight / radius_y + radius * Math.sin(angle12) - childHeight
    child12.style.left = x12 + 'px'
    child12.style.top = y12 + 'px'

    const x13 = parent.clientWidth / radius_x + radius * Math.cos(angle13) - childWidth
    const y13 = parent.clientHeight / radius_y + radius * Math.sin(angle13) - childHeight
    child13.style.left = x13 + 'px'
    child13.style.top = y13 + 'px'

    const x14 = parent.clientWidth / radius_x + radius * Math.cos(angle14) - childWidth
    const y14 = parent.clientHeight / radius_y + radius * Math.sin(angle14) - childHeight
